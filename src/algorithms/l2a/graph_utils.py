...
GraphList = List[Tuple[int, int, float]]  # 每条边两端点的索引以及边的权重 List[Tuple[Node0ID, Node1ID, WeightEdge]]
...
def load_graph_list_from_txt(txt_path: str = 'G14.txt') -> GraphList:
    """
    同时兼容 0-based 和 1-based 索引的 txt：
    - 如果内部出现了节点 0，就认为输入就是 0-based，不做偏移。
    - 否则认为是 1-based，就减 1。
    """
    ...
    for line in edge_lines:
        ...
        u_raw, v_raw = int(u_str), int(v_str)
        if u_raw == 0 or v_raw == 0:
            seen_zero = True
        raw_edges.append((u_raw, v_raw, float(w_str)))

    # 决定是否需要减 1
    if seen_zero:
        # 0-based → 直接使用
        graph_list = [(u, v, w) for (u, v, w) in raw_edges]
    else:
        # 1-based → 转成 0-based
        graph_list = [(u-1, v-1, w) for (u, v, w) in raw_edges]

    # 校验（允许 header 中的 num_nodes ≥ 实际出现的节点数）
    derived = obtain_num_nodes(graph_list)
    assert derived <= num_nodes, (
        f"解析后节点数 {derived} 超过 header 中指定的 {num_nodes}"
    )
    assert num_edges == len(graph_list), (
        f"文件头标注边数 {num_edges} ≠ 实际边数 {len(graph_list)}"
    )
...
def build_adjacency_matrix(graph_list: GraphList, if_bidirectional: bool = False) -> TEN:
    ...
    """
    not_connection = 0  # 选用-1去表示表示两个node之间没有edge相连，不选用0是为了避免两个节点的距离为0时出现冲突
    num_nodes = obtain_num_nodes(graph_list=graph_list)
