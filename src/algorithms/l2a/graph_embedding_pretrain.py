...
from evaluator import Recorder
...
def get_objective(adj_bools, net, mask, criterion, if_train, temperature):
    if if_train:
        net.train()
        _mask = mask
    else:
    ...
    eye_out = net.get_node_classify(tgt=seq_memory, temperature=temperature)
...
def train_graph_net_in_a_single_graph(graph_list: GraphList, args: ConfigGraph, net_path: str, gpu_id: int = 0, temperature: float = 0.1):
    ...
    for repeat_id in range(train_times):
        ...
        objective = get_objective(adj_bools, net, mask, criterion, if_train=True,temperature=temperature)
