...
class ConfigGraph:
    def __init__(self, graph_list: GraphList = None, graph_type: str = 'max_cut', num_nodes: int = 0):
        ...
        self.learning_rate = 2 ** -15  # 优化器的学习率
        self.show_gap = 2 ** 5  # 训练时，打印训练进度的间隔步数

        '''model'''
        self.num_heads = 2
        self.num_layers = 4
        self.mid_dim = num_nodes * 1
...
class ConfigPolicy:
    def __init__(self, graph_list: GraphList = None, graph_type: str = 'max_cut', num_nodes: int = 0):
        ...
        '''train'''
        self.num_sims = 2 ** 4  # LocalSearch 的初始解数量
        ...
        self.num_iters = 6 # 进行迭代搜索的总步数
        self.num_sgd_steps = 2 ** 8  # 每一次根据监督信号进行梯度下降的次数
        self.lambda_entropy = 4  # PPO 策略熵的权重（退火方案会控制策略熵，在一个周期内由entropy_weight*1变化到0）
        self.repeat_times = 1  # PPO 在on-policy训练中，数据重复使用的次数
