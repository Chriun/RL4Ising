...
class GraphTRS(nn.Module):
    ...
    # def get_node_classify(self, seq, mask, temperature):
    #     # 假设这里输出的是logits
    #     logits = self.mlp_classifier(seq)
    #     probs = th.softmax(logits / temperature, dim=-1)
    #     return probs
    
    def get_node_classify(self, tgt, temperature):
        # tgt: target tensor sequence
        tgt = tgt / tgt.std(dim=-1, keepdim=True)
        logits = self.mlp_classifier(tgt)
        probs = th.softmax(logits / temperature, dim=-1)
        return probs
